...
int capMinMax (int val, int min, int max) {
	if(val < minPower && val > 0) {
		val = minPower;
	} else if(val > -minPower && val < 0) {
		val = -minPower;
	}
...
void moveStraightGyro(float inches, const int negitaveMod=dir_forwards, int maxTimeout=250, int maxPower=102, int minPower=17) {
...
void rotateTo (int turnDirection, int targetDegrees, int maxPower=75, int minPower=23, int timeOut=3000) {
