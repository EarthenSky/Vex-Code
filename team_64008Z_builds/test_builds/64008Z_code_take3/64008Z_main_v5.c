...
#pragma config(Motor,  port9,           goalHands,     tmotorVex393_MC29, openLoop)
...
void setLeftRightMoveSpeed(int leftSpeed=0, int rightSpeed=0) {
  //calculate deadzone.
  if(abs(leftSpeed) < 7) { leftSpeed = 0; }
  if(abs(rightSpeed) < 7) { rightSpeed = 0; }
...
task usercontrol {
	...
	while(true) {
		/*Large Goal Arm*/
		if(vexRT[Btn8D] == 1 && SensorValue[handsUp] == 0)	{
			motor[goalHands] = -127;
			goalArmToStack = false;
		} else if(vexRT[Btn8R] == 1 && SensorValue[handsDown] == 0) {
			motor[goalHands] = 80;
			goalArmToStack = false;
		} else {
			motor[goalHands] = 0;
